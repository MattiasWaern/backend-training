...
app.get('/api/citat', (req, res) => {
    ...
    if(kategori ) {
        const filteradeCitat = citat.filter(c => c.kategori === kategori);
        res.json(filteradeCitat);
        res.status(200).json({ message: 'Citat hämtade' });
    } else {
        res.json(citat);
        res.status(400).json({ error: 'Kategori saknas' });
    }
});


app.get('/api/citat?kategori=:kategori', (req, res) => {
